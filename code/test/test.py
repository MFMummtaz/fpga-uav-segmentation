...
class Configs():
    def __init__(self):
        ...
        parser.add_argument('--num-classes', type=int, default=2, help='the classes numbers (default: 19 for cityscapes)')
...
def build_data(args, subset_len=None, sample_method='random'):
    from code.datasets.drone_loader_antiuav_quant import UAVSegmDataset
...
def build_model(args, device):
    ...
    if args.model == "custom":
        ...
        #                size=7, 
        #                padding=1, 
        #                upsample=True).to(device=device)
...
def visulization(args, model, device):
    ...
    # data transforms
    MEAN = (0.31328324, 0.32151696, 0.31460182)
    STD = (0.23343998, 0.24014007, 0.23295579)
    ...
    with torch.no_grad():
        for i, imgName in tqdm(enumerate(image_list)):
            ...
            # scale_image = data_transform(img,(1980, 1080) , MEAN, STD)
            # scale_image = data_transform(img,(tw, th) , MEAN, STD)
            scale_image = data_transform(img, (512, 512), MEAN, STD)
